...
`ti_mean.14` Mean thin-ideal internalization score at age 14

`ti_NA.14` Percent of variables missing from the thin-ideal internalization measure
...
`pressure_lose_sum.167_NA_percent` Percent of variables missing from the pressure to lose weight scale

`pressure_musc_sum.167` Sum of pressure to increase muscle scale at age 14

`pressure_musc_sum.167_NA_percent` Percent of variables missing from the pressure to increase muscle scale
...
`restraint_sum.14` Sum of the restrained eating items at age 14
...
`emo_eat_sum.14` Sum of the emotional eating items at age 14

`emo_eat_sum_d.14` Sum of dichotomous responses for emotional eating items

`emo_eat_NA.14` Percent of variables missing from the emotional eating subscale
...
`wt_shape_concern.14` Sum of weight and shape concern items at age 14
...
`body_sat_mean.14` Body dissatisfaction mean score at age 14

`body_sat_NA.14` Percent of variables missing from the body dissatisfaction measure at age 14
...
`diet_freq.16` frequency respondent went on a diet to lose weight or keep from gaining weight during the past year at age 16

`diet_length.16` Time respondent was on a diet at age 16

`diet_lost.16` Weight the respondent lost due to diet at age 16

`diet_regain.16` Weight the respondent gained back, if applicable, after diet at age 16
...
`fear_wtgain.14` Respondent's fear of weight gain at age 14

[INSERT FEAR NA VARIABLE]
...
`wt_intent.18` Respondent's intent with weight at age 18

[INSERT INTENT NA VARIABLE]

`wt_perception.18` Description of respondent's current weight at age 18

[INSERT PERCEPTION NA VARIABLE]

`wt_valuation.18` Degree to which respondent's weight has made difference to how feel about self in past year at age 18
...
`feel_fat.24` How much respondent has 'felt fat' in the past year at age 24

[INSERT FEELING FAT NA VARIABLE]
